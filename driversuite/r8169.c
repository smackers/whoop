...
// 	SET_ETHTOOL_OPS(dev, &rtl8169_ethtool_ops);
  ...
  struct ethtool_cmd *ecmd;
	struct ethtool_drvinfo *edrinfo;
	struct ethtool_regs *regs;
	struct ethtool_wolinfo *wol;
	struct ethtool_stats *eth_stats;
	
	rtl8169_get_drvinfo(dev, edrinfo);
	rtl8169_get_regs_len(dev);
	rtl8169_get_settings(dev, ecmd);
	rtl8169_set_settings(dev, ecmd);
	rtl8169_get_msglevel(dev);
	rtl8169_set_msglevel(dev, 0);
	rtl8169_get_regs(dev, regs, NULL);
	rtl8169_get_wol(dev, wol);
	rtl8169_set_wol(dev, wol);
	rtl8169_get_strings(dev, ETH_SS_STATS, NULL);
	rtl8169_get_sset_count(dev, ETH_SS_STATS);
	rtl8169_get_ethtool_stats(dev, eth_stats, NULL);
	
	rtl8169_suspend(&pdev->dev);
	rtl8169_resume(&pdev->dev);
	rtl8169_runtime_suspend(&pdev->dev);
	rtl8169_runtime_resume(&pdev->dev);
	rtl8169_runtime_idle(&pdev->dev);
	
	struct ifreq *ifr;
	struct rtnl_link_stats64 *rtnl_stats;
	struct sk_buff *skb;
	netdev_features_t netdev_feature = NETIF_F_RXCSUM;
	
	rtl_open(dev);
	rtl8169_close(dev);
	rtl8169_get_stats64(dev, rtnl_stats);
	rtl8169_start_xmit(skb, dev);
	rtl8169_tx_timeout(dev);
	rtl8169_change_mtu(dev, 0);
	rtl8169_fix_features(dev, NETIF_F_RXCSUM);
	rtl8169_set_features(dev, netdev_feature);
	rtl_set_mac_address(dev, NULL);
	rtl8169_ioctl(dev, ifr, 0);
	rtl_set_rx_mode(dev);
	rtl8169_netpoll(dev);
