      ...
      foreach (var block in impl.Blocks)
      {
        for (int idx = 0; idx < block.Cmds.Count; idx++)
        {
          ...
          foreach (var lhs in assign.Lhss.OfType<MapAssignLhs>())
          {
            ...
            Expr memLocExpr = PointerAliasAnalyser.ComputeRootPointer(impl, lhs.Indexes[0] as IdentifierExpr);
            if (memLocExpr == null)
              continue;
          ...
          foreach (var rhs in assign.Rhss.OfType<NAryExpr>())
          {
            ...
            Expr memLocExpr = PointerAliasAnalyser.ComputeRootPointer(impl, rhs.Args[1] as IdentifierExpr);
            if (memLocExpr == null)
              continue;
        ...
        foreach (var cmd in block.Cmds)
        {
          ...
          else if (cmd is AssignCmd)
          {
            foreach (var pair in (cmd as AssignCmd).Lhss.Zip((cmd as AssignCmd).Rhss))
            {
              ...
              {
                foreach (var expr in PointerAliasAnalyser.GetSubExprs(pair.Item2 as NAryExpr))
                {
